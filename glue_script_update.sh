...
do
  ...
  ### file check ###
  echo "Start checking GlueJobName=${JOB_NAME}, SourceFilePass=${JOB_NAME}/script/${JOB_FILE}."
  cd ${JOB_NAME}/script
  if [ $? -ne 0 ]; then
    echo "Glue Job No such file or directory. JobName(Directory)=${JOB_NAME}, SourceFilePass=${JOB_NAME}/script/${JOB_FILE}."
    exit 2
  fi
  ### deploy ###
